"""
This script contains different implementations for similarities
...
class CkaSimilarityTorch(object):
...
def cos_sim(x1, x2, *args, **kwargs):
    return F.cosine_similarity(x1, x2).mean()

def cka_sim(x1, x2, *args, **kwargs):
    device='cuda:{}'.format(x1.get_device())
    return CkaSimilarityTorch(device).linear_CKA_sim(x1, x2)
